// Test edge case calibration fixes

// Physics simulation function (same as homerunderby.js)
...
// NEW calibration function with edge case handling
function applyCalibration(rawCarryYards, ballSpeedMph, launchAngleDeg, sideAngleDeg, totalSpinRpm, spinAxisDeg) {
    ...
    let angleFactor = 1.0;
    if (launchAngleDeg < 15) {
        const angleBoost = (15 - launchAngleDeg) * 0.031;
        angleFactor = 1.0 + angleBoost;
    }

    // Low spin correction
    let spinFactor = 1.0;
    if (totalSpinRpm < 2000) {
        const spinReduction = (2000 - totalSpinRpm) / 1000 * 0.12;
        spinFactor = 1.0 - spinReduction;
    }
    ...
    if (absSpinAxis > 20) {
        const spinAxisReduction = (absSpinAxis - 20) / 12 * 0.14;
        spinAxisFactor = 1.0 - Math.min(spinAxisReduction, 0.14);
    }
...
// Test data
const tests = [
    { speed: 61.4, vla: 14.3, hla: 1.4, spin: 1059, spinAxis: 24.1, expected: 45.0, name: "Low Angle + High SpinAxis" },
    { speed: 58.6, vla: 21.6, hla: -7.5, spin: 1393, spinAxis: -32.5, expected: 52.7, name: "Low Spin + Very High SpinAxis" },
    { speed: 63.0, vla: 23.7, hla: 2.0, spin: 1494, spinAxis: 5.0, expected: 65.7, name: "Low Spin" },
    { speed: 62.1, vla: 18.3, hla: -5.3, spin: 1609, spinAxis: 7.4, expected: 56.1, name: "Normal" },
    { speed: 64.6, vla: 17.5, hla: -1.5, spin: 1938, spinAxis: 14.1, expected: 59.3, name: "Normal" },
    { speed: 63.5, vla: 20.6, hla: -6.9, spin: 2030, spinAxis: 4.3, expected: 62.9, name: "Normal" },
    { speed: 70.8, vla: 23.3, hla: -3.7, spin: 2101, spinAxis: 0.1, expected: 81.5, name: "Normal" },
    { speed: 67.1, vla: 20.9, hla: -6.7, spin: 2077, spinAxis: 12.6, expected: 70.1, name: "Normal" },
    { speed: 68.5, vla: 24.7, hla: -0.7, spin: 1957, spinAxis: 10.8, expected: 77.8, name: "High Angle" },
    { speed: 69.3, vla: 23.0, hla: -5.8, spin: 1088, spinAxis: -3.9, expected: 77.4, name: "Very Low Spin" },
    { speed: 74.0, vla: 8.3, hla: -4.7, spin: 1276, spinAxis: -1.4, expected: 47.4, name: "Very Low Angle" }
];

console.log('Testing Edge Case Calibration:\n');
...
tests.forEach((test, i) => {
    ...
    const calibrated = applyCalibration(result.rawCarry, test.speed, test.vla, test.hla, test.spin, test.spinAxis);
