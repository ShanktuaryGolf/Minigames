// Test updated calibration against real-world shots
...
function applyCalibration(rawCarryYards, ballSpeedMph, launchAngleDeg, sideAngleDeg, totalSpinRpm, spinAxisDeg) {
    ...
    if (totalSpinRpm > 3800 && absHLA < 15) {
        const spinReduction = (totalSpinRpm - 3800) / 400 * 0.12;
        spinFactor = 1.0 - Math.min(spinReduction, 0.12);
    }

    // High spin axis correction (> 25°)
    let spinAxisFactor = 1.0;
    const absSpinAxis = Math.abs(spinAxisDeg);
    if (absSpinAxis > 25) {
        const spinAxisReduction = (absSpinAxis - 25) / 7 * 0.08;
        spinAxisFactor = 1.0 - Math.min(spinAxisReduction, 0.10);
    }
...
console.log('Updated Calibration Test:\n');
...
shots.forEach((test, i) => {
    ...
    const calibrated = applyCalibration(rawCarry, test.speed, test.vla, test.hla, test.spin, test.spinAxis);
