...
class DartsGame {
    ...
    initNetwork() {
        const gameMode = localStorage.getItem('dartsGameMode');
        if (gameMode !== 'steam') {
            console.log('📍 Darts: Local game mode');
            this.isNetworkGame = false;
            return;
        }

        // Use the global steamNetwork from electron-index-radial.html
        if (typeof steamNetwork !== 'undefined' && steamNetwork.isConnected()) {
            this.isNetworkGame = true;
            this.localSteamId = steamNetwork.getLocalPlayer()?.steamId;
            this.isNetworkHost = steamNetwork.isLobbyHost();
            console.log(`🌐 Darts: Steam mode - ${this.isNetworkHost ? 'HOST' : 'CLIENT'}`);

            // Listen for network messages
            steamNetwork.onMessage((msg) => this.handleNetworkMessage(msg));
        } else {
            console.log('⚠️ Darts: Steam not connected, falling back to local');
            this.isNetworkGame = false;
        }
    }

    handleNetworkMessage(message) {
        if (message.from === this.localSteamId) return;

        switch (message.type) {
            case 'dart_throw':
                const { shotData, playerIndex } = message.data || message;
                if (playerIndex !== undefined) this.currentPlayerIndex = playerIndex;
                this.handleShot(shotData, true); // true = from network
                break;
            case 'game_state':
                console.log('🌐 Darts game state:', message);
                break;
        }
    }

    broadcastDart(shotData) {
        if (!this.isNetworkGame || typeof steamNetwork === 'undefined') return;
        steamNetwork.broadcast({
            type: 'dart_throw',
            data: { shotData, playerIndex: this.currentPlayerIndex }
        });
    }
    ...
    handleShot(shotData, fromNetwork = false) {
        if (!this.gameActive) return;

        // Broadcast to network peers (only for local shots)
        if (!fromNetwork && this.isNetworkGame) {
            this.broadcastDart(shotData);
        }
