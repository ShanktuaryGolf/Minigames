...
// Golf ball physics test script - HYBRID PHYSICS (v4.4.3)
// Uses v4.4.1 for wedges (< 65 mph) and v4.4.2 for irons (65+ mph)
// Run with: node test-hybrid-physics.js
...
function simulateShot(speedMPH, vlaDegs, hlaDegs, backspinRPM, sidespinRPM) {
    ...
    const isWedgeShot = initialSpeedMPH < 65;
    ...
    while (ballInFlight && time < maxTime) {
        ...
        if (isWedgeShot) {
            ...
            // Iron regime spin adjustment (v4.4.2)
            if (currentSpinRPM >= 5000) {
        ...
        if (speed > 0.1 && totalSpinRadS > 1) {
            ...
            if (vlaDegs >= 20) {
                if (isWedgeShot) {
                    liftCoeff *= 1.22; // v4.4.1
                } else {
                    liftCoeff *= 1.22; // v4.4.2 - reduced from 1.25 to prevent over-carry on high-loft irons
                }
            }
        ...
        if (ballPosition.y <= 0.021336 && ballVelocity.y < 0) {
            ...
            if (Math.abs(ballVelocity.y) > 0.5) {
                ...
                if (isWedgeShot) {
                    ...
                    // Iron regime - Moderately reduced bounce for less rollout
                    if (currentLandingSpeed <= 30) {
                        baseBounce = 0.72; // Reduced from 0.75 (4% reduction)
                    } else if (currentLandingSpeed <= 40) {
                        const t = (currentLandingSpeed - 30) / (40 - 30);
                        baseBounce = 0.72 - t * (0.72 - 0.57); // Reduced proportionally
                    } else if (currentLandingSpeed <= 60) {
                        const t = (currentLandingSpeed - 40) / (60 - 40);
                        baseBounce = 0.57 - t * (0.57 - 0.40); // Reduced proportionally
                    } else if (currentLandingSpeed <= 80) {
                        const t = (currentLandingSpeed - 60) / (80 - 60);
                        baseBounce = 0.40 - t * (0.40 - 0.30); // Reduced proportionally
                    } else {
                        baseBounce = 0.30; // Reduced from 0.32 (6% reduction)
                    }
                }
                ...
                if (isWedgeShot) {
                    ...
                    // Iron regime rolling (v4.4.2 - needs less roll)
                    baseFriction = 0.050; // Increased from 0.045 (11% more friction)
                    if (currentRollSpinRPM >= 4000) {
                        baseFriction = 0.078; // Increased from 0.070 (11% more)
                    } else if (currentRollSpinRPM >= 3000) {
                        const t = (currentRollSpinRPM - 3000) / (4000 - 3000);
                        baseFriction = 0.061 + t * (0.078 - 0.061); // Increased proportionally
                    } else if (currentRollSpinRPM >= 2000) {
                        const t = (currentRollSpinRPM - 2000) / (3000 - 2000);
                        baseFriction = 0.050 + t * (0.061 - 0.050); // Increased proportionally
                    }
                }
