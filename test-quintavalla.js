...
// Golf ball physics test script - Quintavalla Model
// Tests against FlightScope data
// Run with: node test-quintavalla.js
...
// Quintavalla Model - Aerodynamic coefficients
function getCdQuintavalla(speed, totalSpinRadS) {
    const spinRatio = (BALL_RADIUS_FT * totalSpinRadS) / speed;
    return 0.171 + 0.62 * spinRatio;
}

function getClQuintavalla(speed, totalSpinRadS) {
    const spinRatio = (BALL_RADIUS_FT * totalSpinRadS) / speed;
    return 0.083 + 0.885 * spinRatio;
}

function getCmQuintavalla(speed, totalSpinRadS) {
    const spinRatio = (BALL_RADIUS_FT * totalSpinRadS) / speed;
    return 0.0125 * spinRatio;
}
...
function simulateShot(speedMPH, vlaDegs, hlaDegs, totalSpinRPM, spinAxisDegs) {
    ...
    while (ballInFlight && time < maxTime) {
        ...
        // Quintavalla Drag
        const Cd = getCdQuintavalla(speed, totalSpinRadS);
        ...
        // Quintavalla Spin Decay
        const Cm = getCmQuintavalla(speed, totalSpinRadS);
        const spinDecayRate = Cm * speed * 0.01;
        totalSpinRadS *= Math.exp(-spinDecayRate * deltaTime);
...
console.log('\n=== QUINTAVALLA MODEL TEST RESULTS ===\n');
...
for (let i = 1; i < lines.length; i++) {
    ...
    console.log(`  Quintavalla:  ${result.carry.toFixed(1)} carry + ${result.roll.toFixed(1)} roll = ${result.total.toFixed(1)} total`);
