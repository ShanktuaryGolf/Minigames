...
console.log('\n=== FLIGHTSCOPE3.CSV TEST RESULTS (v4.4.1 Physics) ===\n');
...
for (let i = 1; i < lines.length; i++) {
    ...
    // Parse HLA - handle "6 R" or "9.7 L" format
    let hlaStr = testData['Launch H (deg)'] || '0';
    let hla = parseFloat(hlaStr);
    if (hlaStr.includes('L')) hla = -Math.abs(hla); // Left is negative
    if (hlaStr.includes('R')) hla = Math.abs(hla);  // Right is positive

    const totalSpin = parseFloat(testData['Spin (rpm)']);

    // Parse Spin Axis - handle "7.4 R" or "39.1 L" format
    let spinAxisStr = testData['Spin Axis (deg)'] || '0';
    let spinAxis = parseFloat(spinAxisStr);
    if (spinAxisStr.includes('L')) spinAxis = -Math.abs(spinAxis); // Left is negative
    if (spinAxisStr.includes('R')) spinAxis = Math.abs(spinAxis);  // Right is positive
